    ...
    <div className="space-y-8 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="space-y-1.5">
          <h1 className="text-3xl lg:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-foreground to-foreground/60 bg-clip-text text-transparent">
            Financial Overview
          </h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Link href="/dashboard/expenses">
            <button className="flex items-center gap-2 h-10 px-4 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition shadow-md shadow-primary/20">
              <Plus className="h-4 w-4" /> Add Expense
            </button>
          </Link>
          <Link href="/dashboard/voice-entry">
            <button className="flex items-center gap-2 h-10 px-4 rounded-xl bg-muted text-foreground font-semibold text-sm hover:bg-muted/80 transition border border-border">
              <Mic className="h-4 w-4" /> Voice Entry
            </button>
          </Link>
          <Link href="/dashboard/social-impact">
            <button className="flex items-center gap-2 h-10 px-4 rounded-xl bg-pink-500/10 text-pink-600 font-semibold text-sm hover:bg-pink-500/20 transition border border-pink-500/20">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Income */}
        <div className="relative overflow-hidden rounded-2xl border border-green-500/20 bg-gradient-to-br from-green-500/10 to-green-500/5 p-5 group hover:border-green-500/40 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
            <div className="w-10 h-10 rounded-xl bg-green-500/20 flex items-center justify-center">
              <TrendingUp className="h-5 w-5 text-green-500" />
            </div>
            <span className="text-xs font-bold text-green-600 bg-green-500/10 px-2 py-0.5 rounded-full flex items-center gap-1">
        ...
        {/* Expenses */}
        <div className="relative overflow-hidden rounded-2xl border border-red-500/20 bg-gradient-to-br from-red-500/10 to-red-500/5 p-5 group hover:border-red-500/40 hover:shadow-lg hover:shadow-red-500/10 transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
            <div className="w-10 h-10 rounded-xl bg-red-500/20 flex items-center justify-center">
              <TrendingDown className="h-5 w-5 text-red-500" />
            </div>
            <span className="text-xs font-bold text-red-600 bg-red-500/10 px-2 py-0.5 rounded-full flex items-center gap-1">
              <ArrowDownRight className="h-3 w-3" /> Spent
            </span>
          </div>
        ...
        {/* Net Balance */}
        <div className={`relative overflow-hidden rounded-2xl border p-5 group transition-all duration-300
          ${summary.balance >= 0
          ...
          <div className="flex items-center justify-between mb-4">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${summary.balance >= 0 ? 'bg-primary/20' : 'bg-red-500/20'}`}>
              <Wallet className={`h-5 w-5 ${summary.balance >= 0 ? 'text-primary' : 'text-red-500'}`} />
            </div>
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${summary.balance >= 0 ? 'text-primary bg-primary/10' : 'text-red-600 bg-red-500/10'}`}>
              Net Balance
            </span>
          </div>
          <p className={`text-2xl font-extrabold ${summary.balance >= 0 ? 'text-primary' : 'text-red-600'}`}>
            ${Math.abs(summary.balance).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
          </p>
          <p className="text-xs text-muted-foreground mt-1">{summary.balance >= 0 ? 'Surplus' : 'Deficit'} this period</p>
        ...
        {/* Savings Rate */}
        <div className="relative overflow-hidden rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-500/10 to-violet-500/5 p-5 group hover:border-violet-500/40 hover:shadow-lg hover:shadow-violet-500/10 transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-2xl font-extrabold text-violet-600">{savingsRate}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="rounded-2xl border bg-card h-full p-5 space-y-3">
            ...
            {[
              ...
              <Link key={href} href={href}>
                <div className={`flex items-center justify-between px-3 py-2.5 rounded-xl ${bg} transition-all cursor-pointer group`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Financial tip card */}
        <div className="rounded-2xl border bg-gradient-to-br from-primary/5 to-violet-500/5 p-6 flex flex-col justify-between">
          ...
          <Link href="/dashboard/insights" className="mt-6">
