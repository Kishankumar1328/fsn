...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const expense = db.prepare('SELECT * FROM expenses WHERE id = ? AND user_id = ?').get(params.id, user.id);
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getDb();
    const existing = db.prepare('SELECT * FROM expenses WHERE id = ? AND user_id = ?').get(params.id, user.id);
    ...
    const updated = db.prepare('SELECT * FROM expenses WHERE id = ?').get(params.id);
